...
class EmployeeInfoUpdateForm(Document):
	def validate(self):
		...
		if self.clear_if_empty == 0 and self._update in ["Identity", "Dependents", "Passports", "Qualifications"]:
			field_name = self._update.lower()
			if len(self.get(field_name)) == 0:
				frappe.throw(_("check clear if empty for empty {}".format(self._update)))
	
	def on_submit(self):
		if self.status not in ["Approved", "Rejected"]:
			frappe.throw(_("Only Document with status 'Approved' and 'Rejected' can be submitted"))
