...
def execute(filters=None):
    ...
    if filters.group_by:
        emp_map, group_by_parameters = get_employee_details(
            filters.group_by, filters.company, filters.branch, filters.department
        )
        holiday_list = []
        ...
        holiday_list = [
            emp_map[d]["holiday_list"] for d in emp_map if emp_map[d]["holiday_list"]
        ]
    ...
    for idx, day in enumerate(days, start=0):
        ...
        for emp in emp_att_map.keys():
            if emp_att_map[emp][idx]:
                ...
                if (
                    "Present" in emp_att_map[emp][idx]
                    or "Work From Home" in emp_att_map[emp][idx]
                ):
    ...
    for emp in employee_map:
        ...
        for day in date_list:
            ...
            if status is None and holiday_map:
                emp_holiday_list = (
                    emp_det.holiday_list
                    if emp_det.holiday_list
                    else default_holiday_list
                )
...
def get_columns(filters):
    ...
    columns += [
        _("Department") + ":Link/Department:108",
        _("Branch") + ":Link/Branch:108",
    ]
    days = []
...
def get_attendance_list(conditions, filters):
    ...
    for d in attendance_list:
        ...
        if d.status == "Absent":
            att_map[d.employee][
                d.day_of_month
            ] = f"<p style=color:#F683AE;>{d.status}</p>"
        if d.status == "Work From Home":
            att_map[d.employee][
                d.day_of_month
            ] = f"<p style=color:green;>{d.status}</p>"
        if d.status == "On Leave":
            att_map[d.employee][
                d.day_of_month
            ] = f"<p style=color:green;>{d.status}</p>"
        if d.status == "Half Day":
            att_map[d.employee][d.day_of_month] = f"<p style=color:blue;>{d.status}</p>"
        if d.status == "Present":
            att_map[d.employee][
                d.day_of_month
            ] = f"<p style='margin:0 ;text-align: left !important; color:#318AD8 '>{d.status}</p><p style='margin:0;color:#ffab00'>{f'In Time: {d.in_time.hour}' if d.in_time else ''} {f':{d.in_time.minute}' if d.in_time else ''}</p><p style='margin:0;color:#ffab00'>{f'Out Time: {d.out_time.hour}' if d.out_time else ''} {f':{d.out_time.minute}' if d.out_time else ''}</p>"
    return att_map
...
def get_conditions(filters):
    ...
    filters["total_days_in_month"] = (
        days_between(filters.get("from"), filters.get("to"))
    ) + 1
