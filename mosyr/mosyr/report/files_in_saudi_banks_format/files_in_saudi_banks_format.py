...
def export_query(date=frappe.utils.today()):
    ...
    if file_format_type == "Excel":
        ...
        if get_bank()[0] == "Al Rajhi Bank" and get_bank()[1] == "Payroll":
            doc = frappe.get_doc(
                {"doctype": "Rajhi", "rajhi_date": frappe.utils.today()}
            )
            doc.insert(
        ...
        if get_bank()[0] == "Al Rajhi Bank" and get_bank()[1] != "Payroll Cards":
            ...
            if company_controller.calendar_accreditation == "Hijri":
                if company_controller.disbursement_type == "Payroll":
                    data["result"][0]["bank_acc_no"] = convert_date(
                        frappe.utils.today()
                    ).replace("-", "")
        ...
        if get_bank()[0] == "Riyadh Bank" and get_bank()[1] == "WPS":
            ...
            doc = frappe.get_doc(
                {"doctype": "Riyadh Bank", "today": frappe.utils.today()}
            )
            doc.insert(
                ignore_permissions=True,
                ignore_links=True,
...
def execute(filters=None):
    ...
    if len(co_controller) > 0:
        company_controller = frappe.get_doc(
            "Company Controller", filters.get("company")
        )
        ...
        if bank_name == "Al Inma Bank" and disbursement_type == "Payroll":
        ...
        elif bank_name == "The National Commercial Bank" and (
            ...
            return get_columns_alrajhi_interchange(), get_data_alrajhi_interchange(
                filters
            )
        elif bank_name == "Al Rajhi Bank" and disbursement_type == "Payroll Cards":
            return get_columns_alrajhi_payroll_card(), get_data_alrajhi_payroll_card(
                filters
            )
...
def get_data_inma_payroll(filters):
    ...
    if filters.get("bank"):
        condition += f" AND emp.employee_bank='{filters.get('bank')}'"
    if filters.get("company"):
        condition += f" AND emp.company='{filters.get('company')}'"
    if filters.get("year"):
        condition += f" AND year(sl.start_date) ='{filters.get('year')}'"
    data = frappe.db.sql(
        f"""
		SELECT
			1 as one1,
    ...
    company = (
        filters.get("company") or frappe.get_doc("Global Defaults").default_company
    )
...
def get_data_inma_wps(filters):
    ...
    if not salary_slip_list:
        return []
    salary_slip = frappe.get_last_doc(
        "Salary Slip", filters={"company": company, "docstatus": 1}
    )
    salary = []
    for d in data:
        id = ""
...
def get_data_riad(filters):
    ...
    agreement_symbol = (
        frappe.get_value("Company Controller", company, "agreement_symbol") or "0"
    )
    num = ""
    ...
    for d in data:
        ...
        del d["empname"]
        other_earnings = (
            flt(d.get("gross_pay" or "0"))
            - flt(d.get("basic" or "0"))
            - flt(d.get("housing_allowance" or "0"))
        )
        other_earnings = "%.2f" % other_earnings
        d.update({"other_earnings": (str(other_earnings).zfill(12))})
    if not data:
        return []
    if not data[0].get("emp_name"):
        return []
    salary_slip_list = frappe.get_list("Salary Slip")
    if not salary_slip_list:
        return []
...
def get_data_sumba(filters):
    ...
    for d in data:
        if not d.get("departement_location"):
            d.update({"departement_location": " " * 20})
        del d["name"]
        other_earnings = (
            flt(d.get("gross_pay" or "0"))
            - flt(d.get("basic" or "0"))
        ...
        if d.get("salary", False):
            ...
            salary = d.get("salary").split(".")[0][1:]
        checksum = _samba_wps_checksum(
            company_controller.company_id, d.get("id_number"), d.get("emp_name"), salary
        )
        ...
        if d.get("basic", False):
            ...
            d.update({"basic": "00000000000"})
        if d.get("housing_allowance", False):
            sl_housing_allowance = (
                d.get("housing_allowance").replace(",", "").replace(".", "")
            )
            d.update({"housing_allowance": sl_housing_allowance})
        else:
            d.update({"housing_allowance": "00000000000"})
        if d.get("dedactions", False):
            sl_dedactions = d.get("dedactions").replace(",", "").replace(".", "")
            d.update({"dedactions": sl_dedactions})
        else:
            d.update({"dedactions": "00000000000"})
...
def get_data_alrajhi_payroll(filters):
    ...
    total_salary = sum(salary)
    cal = ""
    if company_controller.calendar_accreditation == "Gregorian":
        cal = "G"
    if company_controller.calendar_accreditation == "Hijri":
        cal = "H"
    ...
    if str("%.2f" % total_salary).zfill(16):
        salary = (
            (str("%.2f" % total_salary).zfill(16)).replace(",", "").replace(".", "")
        )
    else:
...
def get_data_alrajhi_payroll_card(filters):
    ...
    for d in data:
        ...
        if d.get("other_earnings", False):
            sl_other_earnings = (
                d.get("other_earnings").replace(",", "").replace(".", "")
            )
            d.update({"other_earnings": sl_other_earnings})
        if d.get("dedactions", False):
            sl_dedactions = d.get("dedactions").replace(",", "").replace(".", "")
...
def get_data_alaraby(filters):
    ...
    for d in data:
        other_earnings = (
            d.get("gross_pay" or 0)
            - d.get("basic" or 0)
            - d.get("housing_allowance" or 0)
        )
        d.update({"other_earnings": other_earnings})
    company = (
        filters.get("company") or frappe.get_doc("Global Defaults").default_company
...
def get_columns_inma_payroll():
    return [
        ...
        {"fieldname": "emp_num", "fieldtype": "Data", "width": 150},
        {"fieldname": "name", "fieldtype": "Data", "width": 100},
        {"fieldname": "bank_ac_no", "fieldtype": "Data", "width": 100},
        {
            "fieldname": "yes",
            "fieldtype": "Data",
            "width": 100,
        },
        {"fieldname": "swift_number", "fieldtype": "Data", "width": 100},
        {
            "fieldname": "sa",
            "fieldtype": "Data",
            "width": 50,
        },
        {"fieldname": "month_to_date", "fieldtype": "Data", "width": 100},
        {
            "fieldname": "sar",
            "fieldtype": "Data",
            "width": 50,
        ...
        {"fieldname": "id_number", "fieldtype": "Data", "width": 100},
    ]
...
def get_columns_inma_wps():
    ...
    return [
        ...
        {"fieldname": "emp_num", "fieldtype": "Data", "width": 150},
        {"fieldname": "name", "fieldtype": "Data", "width": 200},
        {"fieldname": "bank_ac_no", "fieldtype": "Data", "width": 100},
        {
            "fieldname": "yes",
            "fieldtype": "Data",
        ...
        {"fieldname": "month_to_date", "fieldtype": "", "width": 100},
        {
            "fieldname": "sar",
            "fieldtype": "Data",
            "width": 50,
        ...
        {"fieldname": "basic", "fieldtype": "Data", "width": 100},
        {"fieldname": "housing_allowance", "fieldtype": "Data", "width": 100},
        {"fieldname": "allowance_trans", "fieldtype": "Data", "width": 100},
        {"fieldname": "dedactions", "fieldtype": "Data", "width": 100},
    ]
...
def get_columns_riad():
    return [
        ...
        {"fieldname": "basic", "fieldtype": "Data", "width": 200},
        {"fieldname": "housing_allowance", "fieldtype": "Data", "width": 200},
        {"fieldname": "other_earnings", "fieldtype": "Data", "width": 200},
        {"fieldname": "dedactions", "fieldtype": "Data", "width": 200},
        {
...
def get_columns_sumba():
    return [
        ...
        {"fieldname": "basic", "fieldtype": "Data", "width": 170},
        {"fieldname": "housing_allowance", "fieldtype": "Data", "width": 170},
        {"fieldname": "other_earnings", "fieldtype": "Data", "width": 170},
        {"fieldname": "dedactions", "fieldtype": "Data", "width": 170},
    ]
...
def get_columns_alrajhi_payroll_card():
    return [
        ...
        {"fieldname": "basic", "fieldtype": "Data", "width": 120},
        {"fieldname": "housing_allowance", "fieldtype": "Data", "width": 120},
        {"fieldname": "other_earnings", "fieldtype": "Data", "width": 120},
        {"fieldname": "dedactions", "fieldtype": "Data", "width": 120},
    ]
...
def get_columns_alaraby():
    return [
        ...
        {"fieldname": "month_to_date", "fieldtype": "Data", "width": 100},
        {"fieldname": "bank_ac_no", "fieldtype": "Data", "width": 150},
        {"fieldname": "first_name", "fieldtype": "Data", "width": 150},
        {"fieldname": "swift_number", "fieldtype": "Data", "width": 150},
        {"fieldname": "month", "fieldtype": "Data", "width": 100},
        {"fieldname": "basic", "fieldtype": "Data", "width": 100},
        {"fieldname": "housing_allowance", "fieldtype": "Data", "width": 100},
        {"fieldname": "other_earnings", "fieldtype": "Data", "width": 100},
        {"fieldname": "dedactions", "fieldtype": "Data", "width": 100},
        {"fieldname": "id_number", "fieldtype": "Data", "width": 150},
        {"fieldname": "co_number", "fieldtype": "Data", "width": 150},
    ]
