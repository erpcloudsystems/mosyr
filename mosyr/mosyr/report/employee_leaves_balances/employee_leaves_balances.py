...
def get_columns() -> List[Dict]:
	return [
		...
		{
			"label": _("Actual Leave"),
			"fieldtype": "float",
			"fieldname": "actual_leave",
			"width": 150,
		},
	]
...
def get_data(filters: Filters) -> List:
	...
	for leave_type in leave_types:
		...
		for employee in active_employees:
			...
			value_of_day = ((new_allocation) / 12) /30
			current_date = datetime.now()
			first_day_of_year = datetime(current_date.year, 1, 1)
			days_since_start_of_year = (current_date - first_day_of_year).days
			row.leaves_allocated = new_allocation
			row.leaves_expired = expired_leaves
			row.opening_balance = opening
			row.leaves_taken = leaves_taken
			row.actual_leave = (value_of_day * days_since_start_of_year) -  (leaves_taken * value_of_day)
