    ...
    def on_update(self):
        ...
        frappe.db.sql(
            """
              ...
              WHERE social_insurance_type='Saudi'""".format(
                flt(self.pension_percentage_on_employee),
                flt(self.pension_percentage_on_company),
            )
        )
        ...
        frappe.db.sql(
            """
              ...
              WHERE social_insurance_type='Non Saudi'""".format(
                flt(self.risk_percentage_on_employee),
                flt(self.risk_percentage_on_company),
            )
        )

        has_new_letter_head = False
        company_name = (
            self.organization_english
            if self.organization_english
            else frappe.db.get_value("Company Id", self.company_id, "company")
        )
        html_header_content = ""
        html_footer_content = ""
        image = ""
        if self.logo:
            ...
            html_header_content = """<div class="container-fluid">
                ...
                self.left_header, self.logo, company_name, self.right_header
            )
