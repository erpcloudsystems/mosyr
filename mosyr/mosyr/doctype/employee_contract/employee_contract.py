...
class EmployeeContract(Document):
    def validate(self):
        ...
        previous_contracts = frappe.db.sql(
            """
						...
						AND employee='{0}' AND (contract_start_date BETWEEN '{1}' AND '{2}' OR contract_end_date BETWEEN '{1}' AND '{2}') """.format(
                self.employee,
                get_date_str(self.contract_start_date),
                get_date_str(self.contract_end_date),
            ),
            as_dict=1,
        )
        ...
        if len(previous_contracts) > 0:
            frappe.throw(
                _(
                    "Employee {} has Valid contract within {} and {}".format(
                        self.employee_name,
        ...
        if (
            self.hiring_start_date < self.contract_start_date
            or self.hiring_start_date > self.contract_end_date
        ):
            frappe.msgprint(
                _(
                    "Hiring Date in contract {} is out of range {} and {}".format(
                        self.name, self.contract_start_date, self.contract_end_date
                    )
                )
            )
    ...
    def create_employee_salary_structure(self):
        ...
        if not employee.date_of_birth:
            get_date_str(getdate("01-01-2000"))
        employee.save()
        frappe.db.commit()
        ...
        if self.allowance_housing == "Cash":
            if cint(employee.include_housing) == 0:
                args = {"salary_component": "Allowance Housing"}
                if self.allowance_housing_schedule == "Monthly":
                    if self.house_amount_type == "Percentage":
                        args.update(
                            {
                                "amount_based_on_formula": 1,
                                "formula": "base * {}".format(
                                    flt(self.allowance_housing_value / 100, 2)
                                ),
                            }
                        )

                        if employee.social_insurance_type == "Saudi":
                            e_insurance_formula = "base + ( base * {} ) )  * pension_on_employee / 100".format(
                                flt(self.allowance_housing_value / 100, 2)
                            )
                            c_insurance_formula = "base + ( base * {} ) )  * pension_on_company / 100".format(
                                flt(self.allowance_housing_value / 100, 2)
                            )
                        else:
                            e_risk_formula = "base + ( base * {} ) )  * risk_on_employee / 100".format(
                                flt(self.allowance_housing_value / 100, 2)
                            )
                            c_risk_formula = "base + ( base * {} ) )  * risk_on_company / 100".format(
                                flt(self.allowance_housing_value / 100, 2)
                            )
                    else:
                        args.update(
                        ...
                        if employee.social_insurance_type == "Saudi":
                            e_insurance_formula = (
                                "( base + {} )  * pension_on_employee / 100".format(
                                    flt(self.allowance_housing_value, 2)
                                )
                            )
                            c_insurance_formula = (
                                "( base + {} )  * pension_on_company / 100".format(
                                    flt(self.allowance_housing_value, 2)
                                )
                            )
                        else:
                            e_risk_formula = (
                                "( base + {} )  * risk_on_employee / 100".format(
                                    flt(self.allowance_housing_value, 2)
                                )
                            )
                            c_risk_formula = (
                                "( base + {} ) * risk_on_company / 100".format(
                                    flt(self.allowance_housing_value, 2)
                                )
                            )
        ...
        if self.allowance_trans == "Cash":
            args = {"salary_component": "Allowance Trans"}
            if self.allowance_period == "Monthly":
                if (
                    self.allowance_trans_amount_type == "Percentage"
                    and flt(self.allowance_trans_value) != 0
                ):
                    args.update(
                        {
                            "amount_based_on_formula": 1,
                            "formula": "base * {}".format(
                                flt(self.allowance_trans_value / 100, 2)
                            ),
                        }
                    )
                else:
        ...
        if self.allowance_phone == "Cash":
            args = {"salary_component": "Allowance Phone"}
            if self.allowance_phone_schedule == "Monthly":
                if (
                    self.allowance_phone_amount_type == "Percentage"
                    and flt(self.allowance_phone_value) != 0
                ):
                    args.update(
                        {
                            "amount_based_on_formula": 1,
                            "formula": "base * {}".format(
                                flt(self.allowance_phone_value / 100, 2)
                            ),
                        }
                    )
                else:
        ...
        if self.allowance_worknatural == "Cash":
            args = {"salary_component": "Allowance Worknatural"}
            if self.allowance_worknatural_schedule == "Monthly":
                if (
                    self.allowance_worknatural_amount_type == "Percentage"
                    and flt(self.allowance_worknatural_value) != 0
                ):
                    args.update(
                        {
                            "amount_based_on_formula": 1,
                            "formula": "base * {}".format(
                                flt(self.allowance_worknatural_value / 100, 2)
                            ),
                        }
                    )
                else:
        ...
        if self.allowance_living == "Cash":
            args = {"salary_component": "Allowance Living"}
            if self.allowance_living_schedule == "Monthly":
                if (
                    self.field_allowance_feed_amount_typ == "Percentage"
                    and flt(self.allowance_living_value) != 0
                ):
                    args.update(
                        {
                            "amount_based_on_formula": 1,
                            "formula": "base * {}".format(
                                flt(self.allowance_living_value / 100, 2)
                            ),
                        }
                    )
                else:
        ...
        if self.allowance_other == "Cash":
            args = {"salary_component": "Allowance Other"}
            if self.allowance_other_schedule == "Monthly":
                if (
                    self.allowance_other_amount_type == "Percentage"
                    and flt(self.allowance_other_value) != 0
                ):
                    args.update(
                        {
                            "amount_based_on_formula": 1,
        ...
        ssa_doc.save()
        ssa_doc.submit()
        return {"ss": ss_doc.name, "ssa": ssa_doc.name}
