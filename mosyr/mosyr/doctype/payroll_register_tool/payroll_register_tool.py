...
class PayrollRegisterTool(Document):
    ...
    def prepare_salary_structures(
        ...
        for employee in employees:
            employee_doc = frappe.db.exists(
                "Employee", employee.get("employee_name", None)
            )
            if not employee_doc:
        ...
        msg += msg_str
        if (
            len(not_active_emps) > 0
            or len(different_comp) > 0
            or len(has_salary_structure) > 0
        ):
            pass
