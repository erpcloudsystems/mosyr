...
from erpnext.setup.install import (
    create_custom_role,
    create_role_permissions_for_doctype,
...
class UsersPermissionManager(Document):
    ...
    def get_permissions(self, user):
        ...
        return {
            ...
            "repage": frappe.db.sql(
                f"SELECT cr.page, cr.report, hr.role FROM `tabCustom Role` cr LEFT JOIN `tabHas Role` hr ON hr.parent=cr.name WHERE hr.role='{user.name}'",
                as_dict=True,
            ),
        }
