...
class NoticePage(Document):
    ...
    def get_date(self):
        ...
        contracts = (
            self.get_contracts(result.get("expire_contracts"))
            if len(result.get("expire_contracts"))
            else []
        )
        ids = (
            self.get_ids(result.get("expire_identities"))
            if len(result.get("expire_identities"))
            else []
        )
        passports = (
            self.get_passports(result.get("expire_passports"))
            if len(result.get("expire_passports"))
            else []
        )
        insurances = (
            self.get_insurances(result.get("expire_Insurances"))
            if len(result.get("expire_Insurances"))
            else []
        )
            ...
            contract_list.append(
                {
                    ...
                    "employee_img": frappe.db.get_value(
                        "Employee", contract_doc.employee, "image"
                    )
                    or None,
                    "department": frappe.db.get_value(
                        "Employee", contract_doc.employee, "department"
                    ),
            ...
            ids_list.append(
                {
                    ...
                    "employee_name": frappe.db.get_value(
                        "Employee", id_doc.parent, "employee_name"
                    ),
                    "employee_img": frappe.db.get_value(
                        "Employee", id_doc.parent, "image"
                    )
                    or None,
                    "department": frappe.db.get_value(
                        "Employee", id_doc.parent, "department"
                    ),
            ...
            passports_list.append(
                {
                    ...
                    "employee_name": frappe.db.get_value(
                        "Employee", passport_doc.parent, "employee_name"
                    ),
                    "employee_img": frappe.db.get_value(
                        "Employee", passport_doc.parent, "image"
                    )
                    or None,
                    "department": frappe.db.get_value(
                        "Employee", passport_doc.parent, "department"
                    ),
