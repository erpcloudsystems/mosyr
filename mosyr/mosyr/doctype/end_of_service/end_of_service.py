...
@frappe.whitelist()
def calculate_end_awards(
    salary,
    number_of_years,
    number_of_months,
    number_of_days,
    type_of_contract,
    reason_of_end_contract,
):
    ...
    if int(number_of_years) >= 5:
        value = (
            (0.5 * float(salary) * 5)
            + (float(salary) * (int(number_of_years) - 5))
            + (float(salary) / 12) * int(number_of_months)
            + (float(salary) / 365) * int(number_of_days)
        )
    if float(number_of_years) < 5:
        value = (
            (0.5 * float(salary) * int(number_of_years))
            + (0.5 * float(salary) / 12) * int(number_of_months)
            + (0.5 * float(salary) / 365) * int(number_of_days)
        )
...
def get_salary(employee):
    ...
    salary = frappe.db.sql(
        """select net_pay, gross_pay, name,  start_date
            from  `tabSalary Slip` where
               employee = %(employee)s and docstatus = 1 and  total_working_days = payment_days order by start_date DESC  """,
        {"employee": employee},
        as_dict=1,
    )
    if salary:
    ...
    if salary:
        earning = frappe.db.sql(
            """select  salary_component, amount from `tabSalary Detail`
    							where parentfield='earnings' and parent= '{0}' """.format(
                str(salary[0].name)
            ),
            as_dict=1,
        )
