...
class MosyrDataImport(Document):
    ...
    def check_link_data(self, doctype, value, filed):
        ...
        if not exist:
            ...
            if doctype in doctypes_with_company_field:
                args.update({"company": company.name})
    ...
    def call_api(self, path, company_id, doctype):
        ...
        try:
            ...
            err = frappe.log_error(f"{e}", f"Import {doctype}s Faield. {status_code}!")
            err_msg = _(f"An Error occurred while Import {doctype}s see")
            err_name_html = (
                f' <a href="/app/error-log/{err.name}"><b>{err.name}</b></a>'
            )
    ...
    def show_import_status(
        ...
        frappe.msgprint(
            msg,
            title=f"{total_data} {doctype} Imported",
            indicator="Cs",
            primary_action={
    ...
    def create_import_log(
        self, doctype, total_data, success, existed, errors, headers, error_msgs
    ):
        ...
        err_log.error_json = json.dumps({"headers": headers, "error_msgs": error_msgs})
        err_log.save()
        frappe.db.commit()
        return err_log.name
    ...
    def import_branches(self, company_id):
        ...
        log_name = self.create_import_log(
            "Branch", total_data, success, existed, errors, headers, error_msgs
        )
    ...
    def import_grades(self, company_id):
        ...
        log_name = self.create_import_log(
            "Employee Grade", total_data, success, existed, errors, headers, error_msgs
        )
        self.show_import_status(
            "Employee Grade",
            total_data,
    ...
    def import_departments(self, company_id):
        ...
        for d in data:
            ...
            did = d.get("key", "")
            if dn != "":
                is_new, exist = self.check_link_data(
                    "Department", dn, "department_name"
                )
                if is_new:
                    success += 1
                else:
                ...
                error_msgs.append([did, _("Missing Class Value")])
        log_name = self.create_import_log(
            "Department", total_data, success, existed, errors, headers, error_msgs
        )
        self.show_import_status(
            "Department",
            total_data,
    ...
    def import_employees(self, company_id):
        ...
        if company_id_for_insurances:
            ...
            comapny_data = frappe.get_list(
                "Company Controller",
                filters={"company": company_id_for_insurances.company},
                fields=["*"],
            )
        ...
        for d in data:
            ...
            for k, v in d.items():
                ...
                if k == "designation" and v != "":
                    is_new, v = self.check_link_data(
                        "Designation", v, "designation_name"
                    )
                ...
                if k == "health_insurance_provider" and v != "":
                    is_new, v = self.check_link_data(
                        "Employee Health Insurance", v, "health_insurance_name"
                    )
                if k == "branch" and v != "":
                ...
                args.update({f"{k}": v})
                if k == "bank":
                    args.update({"salary_mode": "Bank"})

            new_employee.update(args)
            company = company_insurances.get("company", False)
            if company:
                new_employee.update({"company": company})
            new_employee.flags.ignore_mandatory = True
            new_employee.save()
            frappe.db.commit()
            for fta in fiels_to_attach:
                self.download_mosyr_file(
                    "Employee", new_employee.name, "image", fta["path"], fta["name"]
                )
            success += 1

        log_name = self.create_import_log(
            "Employee", total_data, success, existed, errors, headers, error_msgs
        )
        self.show_import_status(
            "Employee",
            total_data,
    ...
    def import_identity(self, company_id):
        ...
        for d in data:
            ...
            for k, v in d.items():
                ...
                if (
                    k in ["issue_date_h", "expire_date_h", "border_entry_date_h"]
                    and v != ""
                ):
                    temp_v = f"{v}".split("-")
                    if len(temp_v) == 3:
            ...
            for att in attachments_data:
                ...
                if att:
                    ...
                    is_downloaded, _file = self.download_mosyr_file(
                        "Employee", employee.name, "id_photo", source_path, att
                    )
    ...
    def import_employee_status(self, company_id):
        path = (
            "https://www.mosyr.io/en/api/migration-employee-status.json?company_id={}"
        )
        ...
        mapped_fields = {"attachement": "attachment"}
        link_fields = []
        ...
        log_name = self.create_import_log(
            "Employee Status", total_data, success, existed, errors, headers, error_msgs
        )
        self.show_import_status(
            "Employee Status",
            total_data,
        ...
        for d in data:
            ...
            for att in attachments_data:
                ...
                if att:
                    source_path = "https://www.mosyr.io/sites/default/files/"
                    is_downloaded, _file = self.download_mosyr_file(
                        "Employee", employee.name, "passport_photo", source_path, att
                    )
                    if is_downloaded and new_idx != -1:
                        employee = frappe.get_doc("Employee", employee.name)
        ...
        for d in data:
            ...
            for att in attachments_data:
                ...
                if att:
                    source_path = "https://www.mosyr.io/sites/default/files/"
                    is_downloaded, _file = self.download_mosyr_file(
                        "Employee", employee.name, "attachment", source_path, att
                    )
                    if is_downloaded and new_idx != -1:
                        employee = frappe.get_doc("Employee", employee.name)
    ...
    def import_contracts(self, company_id):
        ...
        finan_values_lookup = {"none": "None", "covered": "Covered", "cash": "Cash"}
        finan_fields = [
            "allowance_phone",
            "allowance_living",
            "allowance_trans",
            "allowance_worknatural",
            "allowance_housing",
            "allowance_other",
        ]
        ...
        for d in data:
            ...
            con_with_same_key = frappe.get_list(
                "Employee Contract", filters={"key": key}
            )
            if len(con_with_same_key) > 0:
                existed += 1
                new_cont = frappe.get_doc(
                    "Employee Contract", con_with_same_key[0].name
                )
            ...
            for k, v in d.items():
                ...
                if k == "attached_documents" and v != "":
                    if isinstance(v, list):
                        attachments_data = attachments_data + v
                    else:
                        attachments_data.append(v)
                    continue
            ...
            if isinstance(attached_documents, list):
                for ad in attached_documents:
                    ...
                    if is_downloaded:
                        new_cont = frappe.get_doc("Employee Contract", new_cont.name)
                        new_cont.append(
                            "attachments", {"attachment": _file["file_url"]}
                        )
                        new_cont.save()
                        frappe.db.commit()
            frappe.db.commit()
    ...
    def import_letters(self, company_id):
        ...
        for d in data:
            ...
            for att in attachments_data:
                ...
                if att:
                    source_path = "https://www.mosyr.io/sites/default/files/"
                    is_downloaded, _file = self.download_mosyr_file(
                        "Letter", new_letter.name, "attachment", source_path, att
                    )
                    if is_downloaded:
        ...
        frappe.db.commit()
        log_name = self.create_import_log(
            "Letter", total_data, success, existed, errors, headers, error_msgs
        )
        self.show_import_status(
            "Letter",
            total_data,
    ...
    def import_deductions(self, company_id):
        ...
        mapped_fields = {"created": "created_at"}
        ...
        for d in data:
            ...
            ed_with_same_key = frappe.get_list(
                "Employee Deduction", filters={"key": key}
            )
    ...
    def import_overtime(self, company_id):
        ...
        ignored_fields = ["nid", "name"]
        date_fields = ["created_at"]
        mapped_fields = {"overtime_hours_by_1.5": "overtime_hours_by_1_5"}
        ...
        for d in data:
            ...
            ed_with_same_key = frappe.get_list(
                "Employee Overtime", filters={"key": key}
            )
            if len(ed_with_same_key) > 0:
                existed += 1
            ...
            if (by_2 + by_1_5) == 0:
                ...
                args.update({"hour_rate": hour_rate})
    ...
    def import_main_settings(self, company_id):
        ...
        if not company_id_doc:
            ...
            log_name = self.create_import_log(
                "Main Settings", 0, 0, 0, 1, headers, error_msgs
            )
            self.show_import_status(
                "Main Settings", 0, 0, 0, 1, headers, error_msgs, log_name
            )
            return
        ...
        if isinstance(data, list):
            if len(data) == 1:
                ...
                pension_percentage_on_company = main_data.get(
                    "pension_percentage_on_company", ""
                )
                pension_percentage_on_company = pension_percentage_on_company.replace(
                    "%", ""
                )
                pension_percentage_on_employee = main_data.get(
                    "pension_percentage_on_employee", ""
                )
                pension_percentage_on_employee = pension_percentage_on_employee.replace(
                    "%", ""
                )
                risk_percentage_on_company = main_data.get(
                    "risk_percentage_on_company", ""
                )
                risk_percentage_on_company = risk_percentage_on_company.replace("%", "")
                risk_percentage_on_employee = main_data.get(
                    "risk_percentage_on_employee", ""
                )
                ...
                if f"{attendance_service}".lower() in [
                    "active",
                    "suspended",
                    "terminated",
                ]:
                    main_settings.attendance_service = (
                        f"{attendance_service}".capitalize()
                    )

                main_settings.pension_percentage_on_company = flt(
                    pension_percentage_on_company
                )
                main_settings.pension_percentage_on_employee = flt(
                    pension_percentage_on_employee
                )
                main_settings.risk_percentage_on_company = flt(
                    risk_percentage_on_company
                )
                main_settings.risk_percentage_on_employee = flt(
                    risk_percentage_on_employee
                )
                ...
                if isinstance(employee_day_working, list):
                    ...
                    for edw in employee_day_working:
                        ...
                        if edw:
                            main_settings.append(
                                "employee_day_working", {"component": edw}
                            )

                employee_day_annual_vacation = main_data.get(
                    "employee_day_annual_vacation", []
                )
                if isinstance(employee_day_annual_vacation, list):
                    for edw in employee_day_annual_vacation:
                        ...
                        if edw:
                            main_settings.append(
                                "employee_day_annual_vacation", {"component": edw}
                            )

                employee_day_childbirth_vacation = main_data.get(
                    "employee_day_childbirth_vacation", []
                )
                if isinstance(employee_day_childbirth_vacation, list):
                    for edw in employee_day_childbirth_vacation:
                        ...
                        if edw:
                            main_settings.append(
                                "employee_day_childbirth_vacation", {"component": edw}
                            )
                ...
                if isinstance(employee_day_hajj_leave, list):
                    for edw in employee_day_hajj_leave:
                        ...
                        if edw:
                            main_settings.append(
                                "employee_day_hajj_leave", {"component": edw}
                            )
                ...
                if isinstance(emp_day_urgent_leave, list):
                    for edw in emp_day_urgent_leave:
                        ...
                        if edw:
                            main_settings.append(
                                "employee_day_urgent_leave", {"component": edw}
                            )
                ...
                if isinstance(employee_day_sick_leave, list):
                    for edw in employee_day_sick_leave:
                        ...
                        if edw:
                            main_settings.append(
                                "employee_day_sick_leave", {"component": edw}
                            )

                employee_day_benefits_withoutpay_leave = main_data.get(
                    "employee_day_benefits_withoutpay_leave", []
                )
                if isinstance(employee_day_benefits_withoutpay_leave, list):
                    for edw in employee_day_benefits_withoutpay_leave:
                        ...
                        if edw:
                            main_settings.append(
                                "employee_day_benefits_with_out_pay_leave",
                                {"component": edw},
                            )
                ...
                if isinstance(emp_day_death_leave, list):
                    for edw in emp_day_death_leave:
                        ...
                        if edw:
                            main_settings.append(
                                "employee_day_death_leave", {"component": edw}
                            )
                ...
                if isinstance(end_of_services, list):
                    for edw in end_of_services:
                        ...
                        if edw:
                            main_settings.append("end_of_services", {"component": edw})
                ...
                if isinstance(emp_day_wedding_leave, list):
                    for edw in emp_day_wedding_leave:
                        ...
                        if edw:
                            main_settings.append(
                                "employee_day_wedding_leave", {"component": edw}
                            )
                ...
                if isinstance(unaccounted_deductions, list):
                    for edw in unaccounted_deductions:
                        ...
                        if edw:
                            main_settings.append(
                                "unaccounted_deductions", {"component": edw}
                            )
                ...
                if isinstance(banks_type_payroll, list):
                    for edw in banks_type_payroll:
                        ...
                        if edw:
                            ...
                            main_settings.append("banks_type_payroll", {"bank": edw})
                ...
                if isinstance(working_in_weekend, list):
                    ...
                    if (
                        "Saturday" in working_in_weekend
                        or "saturday" in working_in_weekend
                    ):
                ...
                if isinstance(baladiya_license, dict):
                    ...
                    if filename:
                        main_settings = frappe.get_doc(
                            "Company Controller", main_settings.name
                        )
                        is_downloaded, _file = self.download_mosyr_file(
                            "Company Controller",
                            main_settings.name,
            ...
            error_msgs.append(_("Data Error, expected list with one record"))
            log_name = self.create_import_log(
                "Main Settings", "", "", "", 1, headers, error_msgs
            )
            self.show_import_status(
                "Main Settings", "", "", "", 1, headers, error_msgs, log_name
            )
    ...
    def import_leave(self, company_id):
        ...
        success = 0
        existed = 0
        mapped_fields = {"leave_type": "leave_type_name"}
        for d in data:
            d = self.get_clean_data(d)
            ltname = d.get("leave_type", "")
            if ltname != "":
                is_new, exist = self.check_link_data(
                    "Leave Type", ltname, "leave_type_name"
                )
                if is_new:
                    success += 1
                else:
        ...
        log_name = self.create_import_log(
            "Leave Type", total_data, success, existed, errors, headers, error_msgs
        )
        self.show_import_status(
            "Leave Type",
            total_data,
    ...
    def handle_error(self, error_msgs, sucess, errors, data, exists):
        ...
        frappe.msgprint(
            msg,
            title=f"{len(data)} Employess Imported",
            indicator="Cs",
            primary_action={
                "label": _("Export Error Log 2"),
                "client_action": "mosyr.utils.download_errors",
                "args": {"data": "e42237ba5e"},
            },
        )

    def get_error_as_json(self, err):
        if not frappe.db.exists("Mosyr Data Import Log", err):
            return {}
