...
class CashCustodyExpense(Document):
    ...
    def check_valid_spent_value(self):
        if self.estimated_value == 0:
            throw(
                _(
                    "The cash custody <b>{}</b> value has been paid in full".format(
                        self.cash_custody
                    )
                )
            )
        ...
        if self.spent_value > self.estimated_value:
            frappe.throw(
                _(
                    "<b>Spent value {}</b> must be smaller than or equal <b>estimated value {}</b>".format(
                        self.spent_value, self.estimated_value
                    )
                )
            )
    ...
    def add_custody_expense(self):
        ...
        doc.append(
            ...
            {"cash_custody": self.name, "spent_value": self.spent_value},
        )
        doc.save()
        frappe.db.commit()
