...
def get_data(filters):
	...
	if filters.get("from") and filters.get("to"):
		COND += "AND DATEDIFF(date_receipt_g, '{}')>=0 AND DATEDIFF(date_receipt_g, '{}')<=0 ".format(filters.get("from"), filters.get("to"))
	...
	sql = frappe.db.sql(""" SELECT name, responsible_employee, responsible_name, branch,
							recipient_employee, recipient_name, date_receipt_g, custody_value, estimated_value
						FROM `tabCash Custody` 
						WHERE docstatus=1 {} """.format(COND))
	return sql
