...
def get_will_expire_docs():
    ...
    """,
        {"now": now, "diff_day": notify_settings.for_contracts_notice_me_before},
        as_dict=True,
    )
    ...
    """,
        {"now": now, "diff_day": notify_settings.for_ids__notice_me_before},
        as_dict=True,
    )
    ...
    """,
        {"now": now, "diff_day": notify_settings.for_passports_notice_me_before},
        as_dict=True,
    )
    ...
    """,
        {"now": now, "diff_day": notify_settings.for_insurance_notice_me_before},
        as_dict=True,
    )
...
def show_expier_docs():
    ...
    if notify_settings.notify_after_login:
        ...
        if print_msg:
            frappe.msgprint(
                _(msg),
                title=_("Employee Docs that will be expired"),
                indicator="orange",
            )
...
def validate_social_insurance(doc, method):
    comapny_data = frappe.get_list(
        "Company Controller", filters={"company": doc.company}, fields=["*"]
    )
    social_type = (
        "Saudi"
        if f"{doc.nationality}".lower() in ["saudi", "سعودي", "سعودى"]
        else "Non Saudi"
    )
...
def notify_expired_dates(doc, method):
    ...
    for eid in emp.identity:
        if getdate(eid.expire_date) < getdate(today()):
            need_update = True
    if need_update:
        emp.db_set("notify_id", 1, update_modified=False)
    ...
    for epass in emp.passport:
        if getdate(epass.passport_expire) < getdate(today()):
            need_update = True
    if need_update:
        emp.db_set("notify_passport", 1, update_modified=False)
    ...
    for edep in emp.dependent:
        if getdate(edep.insurance_card_expire) < getdate(today()):
            need_update = True
    if need_update:
        emp.db_set("notify_insurance_d", 1, update_modified=False)
...
def check_other_annual_leaves(doc, method):
    if cint(doc.is_annual_leave) == 1 and cint(doc.allow_encashment) == 1:
        leaves = frappe.get_list(
            "Leave Type", filters={"is_annual_leave": 1, "allow_encashment": 1}
        )
        ...
        if len(leaves) >= 1:
            ...
            frappe.throw(
                _("Only One Annual Leave allowed to be encashment check.") + msg_str
            )
        ...
        for log in logs_data:
            ...
            if log_id:
                ...
                if not employee_field_value or not timestamp:
                    errors.append(
                        {"log_id": log_id, "result": "Employee Id or Chickin datetime"}
                    )
                    continue
                employee = frappe.db.get_values(
                    "Employee",
                    {"attendance_device_id": employee_field_value},
                ...
                if employee:
                    ...
                    errors.append(
                        {
                            "log_id": log_id,
                            "result": "no employee found for {}".format(
                                employee_field_value or ""
                            ),
                        }
                    )
                    continue

                frappe.logger("mosyr.biometric").debug(
                    {"resuest_data": " [!] logs_data"}
                )
                ...
                success.append({"log_id": log_id, "result": "success"})
    return {
        "errors": errors,
        "success": success,
    }
...
def get_doctypes(doctype, txt, searchfield, start, page_len, filters):
    result = frappe.db.sql(
        ...
        where doc_name LIKE %(txt)s or parent_name LIKE %(txt)s""",
        {"txt": "%" + txt + "%"},
    )
    return result
...
def get_roles(doctype, txt, searchfield, start, page_len, filters):
    result = frappe.db.sql(
        ...
        where is_custom = 1 and name LIKE %(txt)s  LIKE %(txt)s""",
        {"txt": "%" + txt + "%"},
    )
    return result
...
def validate_remaining_loan(doc, method):
    ...
    if loan_doc.total_amount_paid + doc.amount_paid > loan_doc.total_payment:
        frappe.throw(
            f"Remaining in your loan is {loan_doc.total_amount_remaining}, Not {doc.amount_paid}, Please change the amount paid to {loan_doc.total_amount_remaining}"
        )
    ...
    try:
        ...
        for tidx, row in enumerate(rows, 1):
            ...
            if date_diff(repayment.payment_date, new_payment_date) > 0:
                errors.append({"date": repayment.payment_date, "idx": tidx})

        if len(errors) > 0:
            errors_str = ""
            for err in errors:
                errors_str += (
                    "Choose Date after {} in row {}".format(
                        err.get("date", ""), err.get("idx", "")
                    )
                    + "<br>"
                )
...
def reorder_payments_by_dates(row_parent):
    repayment_schedule = frappe.db.sql(
        f"""
           ...
           WHERE parent='{row_parent}' ORDER BY payment_date""",
        as_dict=1,
    )
