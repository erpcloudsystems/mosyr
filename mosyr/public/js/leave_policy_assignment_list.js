		...
		list_view.page.add_inner_button(__("Bulk Leave Policy Assignment"), function () {
			me.dialog = new frappe.ui.form.MultiSelectDialog({
				...
				},
				{
					...
					onchange: () => {
						if (cur_dialog.fields_dict.assignment_based_on.value === "Leave Period") {
							...
							cur_dialog.set_df_property("leave_policy", "hidden", 0);
							cur_dialog.set_df_property("leave_period", "hidden", 0);
							cur_dialog.set_df_property("effective_to", "hidden", 0);
							cur_dialog.set_df_property("effective_from", "hidden", 0);
							cur_dialog.set_df_property("leave_period", "read_only", 0);
							...
							cur_dialog.set_df_property("effective_to", "hidden", 1);
							cur_dialog.set_df_property("leave_period", "read_only", 1);
							cur_dialog.set_df_property("leave_period", "reqd", 0);
							cur_dialog.set_df_property("leave_period", "hidden", 1);
							cur_dialog.set_df_property("leave_policy", "read_only", 0);
							cur_dialog.set_df_property("leave_policy", "reqd", 1);
				...
				action(employees, data) {
					frappe.call({
						method: 'mosyr.leave_policy_assignment.create_assignment_for_multiple_employees',
						async: false,
						args: {
							employees: employees,
							data: data
