...
model.data_field_options = ("Email", "Name", "Phone", "URL", "Barcode", "Hijri Date")
...
def create_cost_center(
    cost_center_name, company, check_company=False, company_fields=[]
):
    ...
    cost_center = frappe.db.get_value(
        "Cost Center",
        filters={"cost_center_name": cost_center_name, "company": company},
    )
    if cost_center:
        return cost_center
    parent_cc = None
...
def create_account(
    ...
    if check_company and company_field:
        try:
            c_account_name = (
                frappe.get_value("Company", company, f"{company_field}") or None
            )
        except:
            pass
    if c_account_name:
    ...
    parent_account = (
        frappe.db.get_value(
            "Account",
            filters={"account_name": parent_account, "company": company, "is_group": 1},
        )
        or None
    )
